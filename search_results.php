<html>
	...
	<body>
		...
		if(isset($_GET["search"]) && !empty($_GET["search"])) {
			...
			mysql_close();
			if($results_number == 1) {
				$results_info = "<div class='alert alert-success' role='alert'>Votre recherche " . '"' . $search . '"' . " a donn&eacute; " . $results_number . ' r&eacute;sultat.</div>';
			} else {
				if($results_number == 0) {
					$results_info = "<div class='alert alert-danger' role='alert'>Votre recherche " . '"' . $search . '"' . " n'a donn&eacute; aucun r&eacute;sultat.</div>";
				} else {
					$results_info = "<div class='alert alert-success' role='alert'>Votre recherche " . '"' . $search . '"' . " a donn&eacute; " . $results_number . ' r&eacute;sultats.</div>';
				}
			}
		} else {
			$results_info = "<div class='alert alert-danger' role='alert'>Votre recherche " . '"' . $search . '" n\'a donn&eacute; aucun r&eacute;sultat.</div>';
