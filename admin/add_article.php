<html>
	...
	<body>
		...
		<div class="container">
			...
			<br>
			<form role="form" method="post" action="add_article.php" enctype="multipart/form-data">
				<div class="form-group">
					<h3>Titre principal</h3>
					<input type="text" class="form-control" name="title" placeholder="Titre principal">
				</div>
					<center><input type="file" name="header_img" class=""></center>
				<div class="form-group">
		...
		<?php
			if(isset($_POST["title"]) && !empty($_POST["title"]) && isset($_POST["preview_text"]) && !empty($_POST["preview_text"]) && isset($_POST["category"]) && !empty($_POST["category"]) && isset($_POST["article_text"]) && !empty($_POST["article_text"])) {
				...
				// on fait une boucle qui va faire un tour pour chaque enregistrement
					...
					echo "<div class='alert alert-success' role='alert' style='margin-top:20px; margin-left:11.5%; margin-right:11.5%;'>L'article a été publié !</div>";
				// on ferme la connexion à mysql
				mysql_close();
			} else {
				echo "<div class='alert alert-danger' role='alert' style='margin-top:20px; margin-left:11.5%; margin-right:11.5%;'>Tout les champs doivent être remplis.</div>";
			}
